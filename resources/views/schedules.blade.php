            ...
            <div class="p-5">
                ...
                <div v-else>
                    <div class="relative flex items-center mb-3 bg-slate-100 rounded-md p-3 justify-between" v-for="(data, i) in allSchedules" :key="i">
                        ...
                        <div :class="{'border-primary': data.status==='actif', 'border-pending':data.status==='fail', 'border-success':data.status==='success', 'border-pending':data.status==='partial', 'border-pending':data.status==='early'}" class="border-l-2 border-3 pl-4 dark:border-primary">
                            <a class="font-bold" href="#">
                                @{{ data.libelle }}
                            </a>
                            <div class="text-slate-500">@{{ data.date }} @{{ data.start_time }} -- @{{ data.end_time }}</div>
                        </div>
